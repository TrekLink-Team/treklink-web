...
> Approved by: [pending — lead review] · Branch: `features/Implementation_GatewaySync` · Design branch: `features/Design_GatewaySync`
>
...
- [ ] 0.3 Put the Stage A/B scope trade to the supervisor (**Q2**)
  - Team lead's position: build Stage A; **omit Stage B if the supervisor agrees to reduce scope**
  - If Stage B is dropped, the offline NFRs and RQ1/RQ2 must be **formally struck** from the charter — not quietly left unmeasured. Phase 9 is isolated so removal is a clean deletion.
  - _Requirements: `requirements.md` §3 gap note, D-005_
...
- [ ] 4.2 `EpisodeCorrelator.correlate()` — open-Incident lookup, append, advance `lastEventAt`
...
- [ ] 4.7 Hand-off to `IncidentsService` via DI; `gateway-sync` never writes the Incident FSM itself
...
- [ ] 8.6 Confirm AC-01 … AC-10 all demonstrably pass; record AC-11 as Stage B–blocked
...
>
> Gated on Phase 0.3. Everything above is reused unchanged; this phase adds an adapter and a queue in front of it.
>
