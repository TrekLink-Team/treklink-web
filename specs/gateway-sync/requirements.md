...
- **REQ-EVT-12** `[B]`: WHEN a packet arrives on PortNum `PRIVATE_APP` (256) carrying an on-device queue-health report, the system SHALL persist the reported per-tier depth and the enqueued / published / shed / refused counters against the originating device, and SHALL NOT treat it as a field event: it creates no `GatewayEvent` and never reaches episode correlation. *(needs check) The JSON serializer's PortNum coverage listed in `04-firmware-ground-truth.md` §4 does not include `PRIVATE_APP`, so on the Stage A JSON topic this packet may arrive without a decoded payload. If so, the report is readable only on the `/2/e/` protobuf topic (REQ-OPT-03). Logged as a REQUEST in `_handoff/SYNC.md` for the firmware owner.*
...
- [ ] **Q3.** Episode window length for REQ-EVT-05. The 30s steady-state beacon interval sets the floor; a window below about 90s risks splitting one episode across two Incidents when beacons are lost. **Proposed default 300 s** as `incidents.episodeWindowSeconds`; carried into C-002 for a decision before AC-04 is written as a test.
- [ ] **Q4.** Backward grace window for REQ-EVT-08. The SOS position is emitted milliseconds before its text, but mesh reordering can widen that gap. **Proposed default 60 s** as `gatewaySync.retroTagGraceSeconds`; carried into C-002.
...
| `gatewaySync.clockSkewToleranceSeconds` | 300 | DB | yes | REQ-ERR-07, proposal |
