...
Tracked, official ledger per `01-session-based-development-and-ssot.md` §4 and
`08-ai-agent-steering-and-discipline.md` Stage 2.5. Update at real session/phase
boundaries (not every small step — that's what `ignore/docs/current-progress.md` is for;
see `01-conventions/09-doc-driven-scaffold-and-ssot-conventions.md` §1 for how the two relate).
...
- Codebase: `npm install` clean, all 3 packages (`backend`, `gateway`, `frontend`) build and
  typecheck. ESLint 9 + Prettier added (weren't present before). CI workflow added
...
- Docs: full backlog authored in `treklink-docs` (`_docs/03-backlog/`) — 8 epics, 87 user
  ...
  matrix in `01-project-charter.md`. `specs/{module}/requirements.md` etc. in this repo are
